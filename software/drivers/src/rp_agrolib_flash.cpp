// See https://www.makermatrix.com/blog/read-and-write-data-with-the-pi-pico-onboard-flash/ for more information about this!
...
/*
 * Finds the first empty flash page by searching from the end of the
 * search range toward the beginning (from upperLimitOffset to lowerLimitOffset).
 * Start: PICO_FLASH_SIZE_BYTES
 * End: __flash_binary_end (XIP_BASE + BINARY_SIZE) - XIP_BASE 
 */
int Flash::findFirstEmptyPage(uint32_t upperLimitOffset, uint32_t lowerLimitOffset)
{
  int page, addr, first_empty_page = -1;
  int* p;

  int max_pages = (upperLimitOffset - lowerLimitOffset) / FLASH_PAGE_SIZE;
  int pages_per_sector = FLASH_SECTOR_SIZE / FLASH_PAGE_SIZE;

  for (page = 0; page < max_pages; ++page)
  {
    addr = XIP_BASE + upperLimitOffset - ((page + 1) * FLASH_PAGE_SIZE);
    p = (int*) addr;
    // 0xFFFFFFFF cast as an int is -1 so this is how we detect an empty page
    if (*p == -1 && first_empty_page < 0)
    {
...
/*
 * This function programs a single page if possible (if the page is empty) without erasing the entire sector
 */
int Flash::programSinglePage(int flash_offs, uint8_t* data, int size)
{
  ...
  printf("Address start_addr (xip_base + flash_offs): %d\n", flash_offs);
  ...
  flash_range_program(flash_offs, (uint8_t*) buf, FLASH_PAGE_SIZE);
 ...
 */
void Flash::erase(int flash_offs)
{
 ...
 */
void Flash::programMultiplePages(int flash_offs, uint8_t* data, int size)
{
 ...
 */
void Flash::read(int flash_offs, uint8_t* reading, int nbytes)
{
  uint8_t *p;
  int start = XIP_BASE + flash_offs;

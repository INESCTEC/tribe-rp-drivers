...
#include "../include/rp_agrolib_drs0101.h"

void HerkulexServoBus::write(uint8_t *data, uint8_t size) {
    for (uint8_t i = 0; i < size; i++) {
        uart_write_char(uart_id, (char) data[i]);
        // printf("%x\n", data[i]);
    }
}

void HerkulexServoBus::sendPacket(uint8_t id, HerkulexCommand cmd, uint8_t *data,
                                  uint8_t data_len) {
...
bool HerkulexServoBus::sendPacketAndReadResponse(HerkulexPacket &resp, uint8_t id,
                                                 HerkulexCommand cmd, uint8_t *data,
                                                 uint8_t data_len) {
    bool success = false;
    ...
    for (uint8_t attempts = 0; attempts < HERKULEX_PACKET_RETRIES; attempts++) {
        ...
        time_started = (clock_t) time_us_64() / 10000;

        while (!getPacket(resp) &&
               ((((clock_t) time_us_64() / 10000) - time_started) / CLOCKS_PER_SEC * 1000 <
        ...
        if ((resp.error == HerkulexPacketError::None) && (resp.id == id) && (resp.cmd == cmd)) {
            ...
            sleep_ms(HERKULEX_PACKET_RESEND_DELAY);
        }
    }
...
void HerkulexServoBus::update() {
    ...
    if (uart_is_readable(uart_id) > 0) {
        ...
        while (uart_is_readable(uart_id)) {
            char aux;
            uart_read_char(uart_id, &aux);
            m_rx_buffer[m_rx_buffer_index] = (uint8_t) aux;
            m_rx_buffer_index = m_rx_buffer_index + 1;
            m_last_serial = (clock_t) time_us_64() / 10000;
        }

        // remove characters before header
        while (m_rx_buffer_index >= 2) {
        ...
        if (m_rx_buffer_index >= 9) {  // 7 (min packet length) + 2 (status data)
            uint8_t packet_len = m_rx_buffer[2];
            if (m_rx_buffer_index >= packet_len) {
                processPacket(false);
                return;
            }
        }
    ...
    if (m_rx_buffer_index > 0) {
        clock_t now = (clock_t) time_us_64() / 10000;
        double executionTime = (double) (now - m_last_serial) / CLOCKS_PER_SEC;

        if (executionTime * 1000000 > HERKULEX_PACKET_RX_TIMEOUT) {
            processPacket(true);
            m_last_serial = now;
        }
    }
...
void HerkulexServoBus::processPacket(bool timeout) {
    ...
    if (m_rx_buffer_index >= 9) {
        // check length field
        if (m_rx_buffer[2] > m_rx_buffer_index) {
            bytes_to_process = m_rx_buffer_index;
    ...
    while (bytes_to_process > 0) {
        uint8_t b = m_rx_buffer[m_rx_buffer_index];
        bytes_to_process--;

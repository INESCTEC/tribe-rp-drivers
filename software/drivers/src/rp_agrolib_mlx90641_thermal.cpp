...
{ 
	...
	Emissivity=0.98;                      // Emissivity coefficient (default: 1)
...
bool MLX90641::readEEPROMBlock(uint16_t startAddr, uint16_t numWords, uint16_t *dest) {
  ...
  for (uint16_t i = 0; i < numWords; i++) {
        ...
        if (reg_read16(I2C_ID, I2C_ADDR, startAddr + i, data, 2) < 2) {
            return false;
        }

        //Big endian to little endian conversion: the first byte read is the high byte, and the second byte is the low byte
        dest[i] = ((uint16_t)data[0] << 8) | data[1];
        
        //Small delay between reads to avoid overwhelming the I2C bus
        sleep_us(5);
  }
  return true;
}
...
bool MLX90641::isNewDataAvailable() {
    ...
    if (reg_read16(I2C_ID, I2C_ADDR, STATUS_ADDR, data, 2) < 2) {
        return false; 
    }

    uint16_t status = (uint16_t)(data[0] << 8) | data[1];

    // 3. Check if the new data available bit (bit 3) is set in the status register
    return (status & (1 << 3)) != 0;
}
...
bool MLX90641::clearNewDataBit() {
    ...
    // Clear the new data available bit (bit 3) by writing back the status with that bit cleared
    uint16_t newStatus = status & ~(1 << 3);
    uint8_t newData[2] = {static_cast<uint8_t>((newStatus >> 8) & 0xFF), static_cast<uint8_t>(newStatus & 0xFF)};

    // Write the new status back to the STATUS register
    return reg_write16(I2C_ID, I2C_ADDR, STATUS_ADDR, newData, 2) == 2;
}
...
uint16_t MLX90641::readAddr_unsigned(const uint16_t readByte) {
  ...
  if(reg_read16(I2C_ID, I2C_ADDR, readByte, data, 2) < 2) {
    return 0;}
...
/*std::string MLX90641::float2exp(float num, byte sigDigits) {
  if (num == 0) return "0.00e+0";
  if (std::isnan(num)) return "NaN";
  int exponent = floor(std::log10(std::abs(num)));   //find order
  float scaled = num / std::pow(10, exponent);  //scale #
  //Round scaled to (sigDigits-1) decimal places:
  float rounded = std::round(scaled * std::pow(10, sigDigits - 1)) / std::pow(10, sigDigits - 1);
...
char buffer[32];
    // %.*f controla o número de casas decimais dinamicamente
    snprintf(buffer, sizeof(buffer), "%.*fe%s%d", 
             sigDigits - 1, 
             rounded, 
             (exponent >= 0 ? "+" : ""), 
             exponent);
...
// to retrieve pixel address, subpage 
...
bool MLX90641::setRefreshRate(uint8_t rate) {
  ...
  data[0] = (uint8_t)(config >> 8);   // High Byte
  data[1] = (uint8_t)(config & 0xFF); // Low Byte

  if (reg_write16(I2C_ID, I2C_ADDR, CTRL_ADDR, data, 2) < 2) {
      return false; 
  }
...
void MLX90641::printFrame(float *Tdat, float Tamb) {
    int index = 0;
    
    // 1. Primeiro valor: Temperatura Ambiente (sem vírgula antes)
    int written = snprintf(&frameBuffer[index], FRAME_BUFFER_SIZE - index, "%.2f", Tamb);
    index += written;

    // 2. Restantes valores: 192 pixéis (cada um com vírgula antes)
    for (int i = 0; i < 192; i++) {
        written = snprintf(&frameBuffer[index], FRAME_BUFFER_SIZE - index, ",%.1f", Tdat[i]);
        if (written <= 0) break;
        index += written;
        if (index >= FRAME_BUFFER_SIZE - 10) break;
    }
    ...
    fwrite(frameBuffer, 1, index, stdout);
    fflush(stdout);
}

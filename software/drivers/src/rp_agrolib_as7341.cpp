...
void AS7341::errorChannels(Channels * channels) {
    channels->F1 = channels->F2 = channels->F3 = channels->F4 = -1;
    channels->F5 = channels->F6 = channels->F7 = channels->F8 = -1;
    channels->CLEAR = channels->NIR = -1;
}

bool AS7341::ReadLight(Channels * channels) {
  bool isEnabled = true;
  ...
  while (isEnabled) {
    if ((time_us_32() - t_start) > timeout_us) {
        printf("[AS7341] Timeout SMUXEN (F1-F4)\n");
  ...
  uint32_t int_time_us = GetIntTime();
  if (int_time_us > 20000000) {
    int_time_us = 20000000;  // fallback: 20s
  }
  uint32_t t_avalid_timeout = int_time_us * 2 + 50000;
  
  t_start = time_us_32();
    while (!isDataReady) {
      if ((time_us_32() - t_start) > t_avalid_timeout) {  // 500ms — depende do integration time
          printf("[AS7341] Timeout AVALID (F1-F4)\n");
          errorChannels(channels);
          return false;
      }
  ...
  while (isEnabled) {
    if ((time_us_32() - t_start) > timeout_us) {
        printf("[AS7341] Timeout SMUXEN (F5-F8)\n");
        errorChannels(channels);
        return false;
    }
    isEnabled = getSmuxEnabled();
  ...
  t_start = time_us_32();
  while (!isDataReady) {
    if ((time_us_32() - t_start) > t_avalid_timeout) {
        printf("[AS7341] Timeout AVALID (F5-F8)\n");
        errorChannels(channels);
        return false;
    }
    isDataReady = getIsDataReady();
...
void AS7341::FlickerDetection(bool enable) {
  if (enable) {
    ...
    while (isEnabled) {
        if ((time_us_32() - t_start) > timeout_us) {
            printf("[AS7341] Timeout SMUXEN (Flicker)\n");
            return;
        }
        isEnabled = getSmuxEnabled();
    }
    ...
    //            writeRegister(uint8_t(0xD7), uint8_t(0x21)); //33 default value, function for setting for Fd_sample and Fd_compare_value
    //
    //            writeRegister(uint8_t(0xD8), uint8_t(0x68)); //104 default value, function for setting for Fd_time lower bit(7:0)
    //
    //            writeRegister(uint8_t(0xDA), uint8_t(0x49)); //73 default value, function for setting for fd_gain and fd_time higher bit(10:8)
...
bool AS7341::detect_overflow(Channels * channels){
    bool flag = false;
    if(channels->F1 == 65535) flag = true;
    else if(channels->F2 == 65535) flag = true;
    else if(channels->F3 == 65535) flag = true;
    else if(channels->F4 == 65535) flag = true;
    else if(channels->F5 == 65535) flag = true;
    else if(channels->F6 == 65535) flag = true;
    else if(channels->F7 == 65535) flag = true;
    else if(channels->F8 == 65535) flag = true;
    else if(channels->NIR == 65535) flag = true;
...
uint16_t AS7341::determine_max_value(Channels * channels){
    uint32_t max;
    max = channels->F1;
    if(channels->F2 > max) max = channels->F2;
    if(channels->F3 > max) max = channels->F3;
    if(channels->F4 > max) max = channels->F4;
    if(channels->F5 > max) max = channels->F5;
    if(channels->F6 > max) max = channels->F6;
    if(channels->F7 > max) max = channels->F7;
    if(channels->F8 > max) max = channels->F8;
    if(channels->NIR > max) max = channels->NIR;
    //printf("max: %d", max);

    return max;
}

void AS7341::autoTimeReadLight(Channels * channels){ //varies between 0.18s (where the ADC reaches full scale) and 1s
...
void AS7341::F1F4_Clear_NIR()
{
  ...
  writeRegister(uint8_t (0x05), uint8_t (0x42)); // F4 left connected to ADC3/f2 left connected to ADC1
...
void AS7341::ReadStatus(STATUS * sts){
  ...
  uint8_t status2 = readRegister(uint8_t(0xA3));
